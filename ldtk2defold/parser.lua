...
local function read_config(path)
	...
	if not file then
		error("Error opening config file: " .. file)
	end
...
function parser.parse(root, identifier, text, config_file)
			...
			if tileset_path then
				...
				editor.transact({
					...
					editor.tx.set(
						resource_path,
						"collision",
						config.assign_collision
								and (config.collisions and config.collisions[tileset.identifier] or tileset_path)
							or tileset_path
					),
				})
	...
	for _, level in ipairs(data.levels) do
		...
		if not tree[world_id] then
			...
			table.insert(
				txns,
				editor.tx.add(collection_path, "children", {
					type = "go",
					id = world_id,
					components = {
						{
			...
			if not tree[world_id].components.iid then
				table.insert(
					txns,
					editor.tx.add(tree[world_id].__ref, "components", {
						type = "component-reference",
						id = "iid",
						path = scripts.iid,
						__iid = level.iid,
		...
		for _, layer in ipairs(level.layerInstances) do
			...
			if layer_type == "Entities" then
				...
				local tilemap_path = create_tilemap(layer, tilesets[layer.__tilesetRelPath], tilemap_root, txns)
				...
				for i = 1, #value do
					...
					if children_tree[map.id] then
						...
						if components[map.id] then
							table.insert(txns, editor.tx.set(components[map.id], "position", { 0, 0, 0 }))
							table.insert(txns, editor.tx.set(components[map.id], "path", map.path))
						else
							table.insert(
								txns,
								editor.tx.add(children_tree[map.id].__ref, "components", {
									type = "component-reference",
									id = map.id,
									path = map.path,
								})
						...
						table.insert(
							txns,
							editor.tx.add(tree[key].__ref, "children", {
								type = "go",
								id = map.id,
